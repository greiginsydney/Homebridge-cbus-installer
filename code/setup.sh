...
step1 ()
{
	...
	then
		...
		if [[ ":$PATH:" == *":/usr/local/node-v11.15.0-linux-armv6l/bin:"* ]]; then
			echo "Already present in the PATH"
		else
			sh -c "echo export PATH=$PATH:/usr/local/node-v11.15.0-linux-armv6l/bin >> /etc/profile"
			source /etc/profile
